...
2. 在 `android/build.gradle(.kts)` **已有的第一个 `subprojects {` 块内**插入 `afterEvaluate { ... }`，对所有 Android library/application 模块通过反射调用 `setCompileSdk`/`setCompileSdkVersion(Int)` 强制 `compileSdk 35`；不得在文件末尾追加新的 `subprojects` 块，否则 Gradle 报 `Cannot run Project.afterEvaluate(Action) when the project is already evaluated`
