...
- [x] Task 1: 移除 build-hap job 及 release 引用
...
- [x] Task 2: cargo binstall 替换 cargo install
...
- [x] Task 3: codegen 命令加 --config 标志
...
- [x] Task 4: 桌面端 Rust cdylib 编译并复制到产物目录
...
- [x] Task 5: Android Rust 库编译并复制到 jniLibs
...
- [x] Task 6: 文档同步
  - [x] 6.1: `AGENTS.md` 移除 HAP 相关陷阱条目（HarmonyOS HAP 构建 / gitcode fork / OpenHarmony SDK / hvigor / ohos / aarch64-unknown-linux-ohos target 等条目）
  - [x] 6.2: `AGENTS.md` 新增陷阱：frb v2 不自动编译桌面/Android Rust cdylib，CI 需显式 `cargo build --release` + 复制共享库到产物目录；Android 需 `cargo-ndk` + jniLibs
  - [x] 6.3: `AGENTS.md` 新增提示：`flutter_rust_bridge_codegen generate` 必须在 `app/` 目录下执行（config 文件 `app/flutter_rust_bridge.yaml`）；CI 加 `--config` 标志防御
  - [x] 6.4: `AGENTS.md` 新增 cargo binstall 约定：CI 用 `cargo binstall --no-confirm` 替代 `cargo install` 加速
