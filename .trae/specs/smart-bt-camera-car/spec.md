...
2. **移动端范围**：仅 Android。iOS 需 macOS 签名环境，暂不纳入流水线（Flutter 工程仍保留 ios 目录占位；后续已通过 `.gitignore` 排除，CI 不构建 iOS）。
...
1. `firmware.yml`：PlatformIO 编译固件，使用 `esptool.py --chip esp32s3 merge-bin` 合并 bootloader+partitions+boot_app0+firmware，产物 `firmware-merged.bin` 可直接烧录到 0x0
