...
- [ ] Task 3: 提交变更
  - [ ] 3.1 使用 `git add` 暂存 `.github/workflows/app.yml`、`AGENTS.md`、`CHANGELOG.md`。
  - [ ] 3.2 使用 Conventional Commits 提交：`fix(ci/app): 注入 _SILENCE_EXPERIMENTAL_COROUTINE_DEPRECATION_WARNINGS 修复 Windows 构建`（若 docs 变更需同步提交或拆分为 `docs(agents)` / `docs(changelog)`，优先一次提交）。
