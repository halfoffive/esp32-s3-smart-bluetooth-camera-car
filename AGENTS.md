...
- `#[frb(sync)]` / `#[frb(opaque)]` 等 frb v2 属性宏不会由 codegen 自动注入到用户源文件，使用该属性的模块（如 `api/mod.rs` / `image.rs`）必须显式 `use flutter_rust_bridge::frb;`，否则 `cargo doc --no-deps --all-features` 报 `cannot find attribute 'frb' in this scope`。
...
- **Visual Studio 2026 / MSVC 14.51 协程弃用**：VS 2026 起 MSVC 14.51 将 `<experimental/coroutine>` 标记为已弃用，`permission_handler_windows` 仍会引入该头文件并触发 STL1011 静态断言。CI 中已在 `.github/workflows/app.yml` 的 `Build Windows desktop` 步骤注入环境变量 `CL=/D_SILENCE_EXPERIMENTAL_COROUTINE_DEPRECATION_WARNINGS` 抑制弃用错误；本地使用 VS 2026 执行 `flutter build windows --release` 前也需设置同一环境变量，否则构建失败。
...
- **Android APK 必须先 `flutter_rust_bridge_codegen integrate`**：`app/android/` 不在版本控制中，每次 CI 由 `flutter create .` 重新生成，原生 `android/` 目录不含 Rust 编译配置。必须在 `flutter create .` 之后、`flutter build apk` 之前执行 `flutter_rust_bridge_codegen integrate`（`app.yml` 的「Integrate flutter_rust_bridge (Android)」步骤，`if: matrix.flutter_target == 'apk'`），向 `android/app/build.gradle` 注入 `externalNativeBuild` CMake + NDK 配置，使 Gradle 自动编译 `rust/` crate 并打包 `lib/<abi>/librust_lib.so` 进 APK。缺失该步，APK 不含 `librust_lib.so`，运行时 `dlopen failed: library "librust_lib.so" not found`，App 进入 `_InitErrorScreen`。iOS 由 Xcode 处理，不在 CI 范围内。**注意**：`integrate` 以模板覆盖方式注入 `rust_builder/`，同时创建模板文件 `src/api/mod.rs` 与 `src/api/simple.rs`；因此 Rust 入口必须使用目录式 `src/api/mod.rs` 而非平铺 `src/api.rs`，否则两者并存触发 E0761 重复模块错误。**`integrate` 还会覆写 `lib/main.dart`**（将原内容注释掉，注入模板 demo 代码 import `simple.dart` + 调用 `greet`），CI 中 `integrate` 后须 `git checkout -- lib/main.dart` 恢复真实入口文件，并清理模板演示文件 `rust/src/api/simple.rs`、`lib/src/rust/api/simple.dart`、`integration_test/simple_test.dart`、`test_driver/integration_test.dart`。**此外**：cargokit 的 `rust_builder/cargokit/gradle/plugin.gradle` 使用 `project.exec {}`，Gradle 9（Flutter 3.44+ stable 捆绑）已移除该方法，须在 `integrate` 后用 Python 脚本 patch 为 `ExecOperations` 注入式调用（`@Inject abstract ExecOperations getExecOperations()` + `getExecOperations().exec {}`），否则报 `Could not find method exec() for arguments [...] on project`。
...
- **Windows 蓝牙关闭时 `FlutterBluePlus.adapterState` 可能抛错**：Windows 上蓝牙未开启或飞行模式时，`adapterState` 流可能在构造阶段直接抛出同步异常，也可能在异步监听中抛异常，而非仅返回 `BluetoothAdapterState.off`。`BleController` 构造时用 `try/catch` 包裹 `FlutterBluePlus.adapterState.listen(...)` 并注册 `onError`，将内部状态回退为 `BluetoothAdapterState.unknown` 并通过 `BleState.errorMessage` 中文提示用户；缺失该防护会导致未处理异常进入 Zone 错误处理器，App 可能卡死或 UI 冻结。`DeviceConnectionScreen` 在 Windows 下提供「打开蓝牙设置」按钮，通过 `Process.run('cmd', ['/c', 'start', 'ms-settings:bluetooth'])` 深链打开系统蓝牙设置。
...
- **Flutter 侧**：Material Design 3 **默认配色**（`useMaterial3: true`，不设 `colorSchemeSeed` / 自定义种子色），结构色一律取自 `Theme.of(context).colorScheme`；**深浅色默认跟随系统**（`ThemeMode.system`），用户可在设置页切换 系统/浅色/深色（持久化到 `shared_preferences` 键 `car_theme_mode`）；状态语义色（正常/警告/危险）使用 Material 默认色（`Colors.green`/`Colors.amber`/`colorScheme.error`），由 `HudStatus` 承载，不随主题变化。Riverpod 状态管理。
  ...
  - **自定义禁用按钮须显式设置 M3 disabled 色**：若用 `FilledButton` 实现启用/禁用切换，禁止通过 `AnimatedOpacity`/`AnimatedContainer` 叠加透明背景来模拟禁用态；应在 `FilledButton.styleFrom` 中显式指定 `disabledBackgroundColor = colorScheme.onSurface.withValues(alpha: 0.12)` 与 `disabledForegroundColor = colorScheme.onSurface.withValues(alpha: 0.38)`，避免不同平台默认 `disabled` 样式差异导致文字/图标不可读。
