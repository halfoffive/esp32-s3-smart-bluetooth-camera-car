...
- CI 中 Android compileSdk patch 步骤应通过 `working-directory: app` 在 Flutter 项目根目录内执行，引用 `android/app/build.gradle` 与 `android/build.gradle` 相对路径；不要在仓库根目录下使用 `app/android/...` 绝对路径。
