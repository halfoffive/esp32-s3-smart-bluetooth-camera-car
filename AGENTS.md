...
- **CI 安装 cargo bin 一律走 binstall**：`.github/workflows/app.yml` 中 `cargo-expand` 与 `flutter_rust_bridge_codegen` 都用 `cargo binstall -y --force <crate>@<version>`（先 `cargo-bins/cargo-binstall@v1.9.0` action 装 binstall 本体，**必须 pin 到具体 tag**，不要用 `@main` —— 后者供应链风险），秒级完成；不要回退到 `cargo install --locked`（从源码编译数分钟）。若某个 crate 在 crates.io 未发布 binstall 兼容的预编译产物，binstall 会自动 fallback 到 `cargo install`。
