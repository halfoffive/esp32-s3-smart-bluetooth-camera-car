...
- **frb v2 不自动编译 Rust cdylib**：`flutter_rust_bridge` Dart 包仅提供 FFI 加载逻辑（`DynamicLibrary.open` / `System.loadLibrary`），不负责编译 Rust 库。CI 需在 `flutter build <platform> --release` 后显式 `cd app/rust && cargo build --release`，并将共享库复制到产物目录：Windows `rust_lib.dll` → `build/windows/x64/runner/Release/`；Linux `librust_lib.so` → `build/linux/x64/release/bundle/lib/`；macOS `librust_lib.dylib` → `.app/Contents/Frameworks/`。Android 需用 `cargo-ndk`（`cargo binstall --no-confirm cargo-ndk`）编译 `arm64-v8a` 并输出到 `android/app/src/main/jniLibs/`。桌面平台目录（`app/{windows,linux,macos}/`）由 `flutter create .` 生成且 gitignored，无法在版本控制中维护 CMake 集成，故采用 CI 显式编译+复制方案。
- `flutter_rust_bridge_codegen generate` **必须在 `app/` 目录下执行**：配置文件 `app/flutter_rust_bridge.yaml` 是 codegen 的输入，从其它目录执行会报 `Please provide rust_input (via config file or command line)` panic。CI 命令为 `cd app && flutter_rust_bridge_codegen generate --config flutter_rust_bridge.yaml`，`--config` 标志为防御性指定。
- **CI 工具链用 `cargo-binstall` 替代 `cargo install`**：`cargo install` 从源码编译 `cargo-expand` / `flutter_rust_bridge_codegen` / `cargo-ndk` 耗时 3-5 分钟；`cargo binstall --no-confirm <pkg> --version <ver>` 下载预编译二进制仅需 ~30 秒。CI 需先通过 curl 官方脚本安装 `cargo-binstall` 本体。
